...
// One-time route to send "disregard" emails for erroneously sent payment reminders
...
export async function POST(request: NextRequest) {
  ...
  for (const r of recipients) {
    try {
      ...
      results.push({ name: r.name, email: r.email, status: 'sent', id: result.data?.id });
    } catch (e) {
      results.push({ name: r.name, email: r.email, status: 'failed', error: (e as Error).message });
    }
  }
